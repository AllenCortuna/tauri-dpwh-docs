...
const SQLiteManager: React.FC = () => {
  ...
  const exportDatabase = async () => {
    ...
    try {
      ...
      errorToast("Failed to export database. See console for details.");
    } finally {
  ...
  const importDatabase = async () => {
    ...
    try {
      ...
      // Clear existing data
      await db.execute("DELETE FROM contractors");
      ...
      for (const contract of importData.contracts) {
        ...
        const placeholders = Object.keys(contractData).map(() => "?").join(", ");
      ...
      errorToast("Failed to import database. See console for details.");
    } finally {
      setExportImportLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4">
      ...
      <div className="mt-8 text-xs text-gray-500 max-w-md text-center">
        <p>
          This utility allows administrators to manage the SQLite database: reset to initial state,
          export data for backup, or import data from a backup file.
